...
<%
    ...
    String searchedWordEncoded = (String) request.getAttribute("searchedWordEncoded");
...
<div class="wrapper">
    ...
    <nav id="navbar" class="navbar navbar-expand-md navbar-light bg-light sticky-top shadow">
        ...
        <div id="navbar-content" class="collapse navbar-collapse align-content-stretch justify-content-between">
            <ul class="navbar-nav">
                <li class="nav-item">
                    <a class="nav-link board0"
                       href="/search?searchType=<%=searchType%>&searchedWord=<%=searchedWordEncoded%>">전체 검색</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link board1"
                       href="/search?board=1&searchType=<%=searchType%>&searchedWord=<%=searchedWordEncoded%>">자유
                        게시판</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link board2"
                       href="/search?board=2&searchType=<%=searchType%>&searchedWord=<%=searchedWordEncoded%>">유머
                        게시판</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link board3"
                       href="/search?board=3&searchType=<%=searchType%>&searchedWord=<%=searchedWordEncoded%>">정보
                        게시판</a>
                </li>
            </ul>
            ...
            <ul class="pagination">
                ...
                <li class="page-item">
                    <a class="page-link, page-prev"
                       href="search?board=<%=board%>&curPage=<%=postCntStart - 1%>&&searchType=<%=searchType%>&searchedWord=<%=searchedWordEncoded%>">이전</a>
                </li>
                <% } %>
                ...
                <li class="page-item">
                    <a class="page-link"
                       href="search?board=<%=board%>&curPage=<%=i%>&&searchType=<%=searchType%>&searchedWord=<%=searchedWordEncoded%>"><%=i%>
                    </a>
                    <% } %>
                </li>
                <% } %>
                ...
                <li class="page-item">
                    <a class="page-link page-next"
                       href="search?board=<%=board%>&curPage=<%=postCntStart + 10%>&&searchType=<%=searchType%>&searchedWord=<%=searchedWordEncoded%>">다음</a>
                </li>
                <% } %>
            </ul>
        </nav>
                ...
                <div class="row">
                    <div class="col-3 col-lg-2">
                        <select id="searchType" name="searchType" class="form-control">
                            <option id="type1" value="1">제목 + 내용</option>
                            <option id="type2" value="2">제목</option>
                            <option id="type3" value="3">내용</option>
                            <option id="type4" value="4">작성자</option>
                        </select>
                    </div>
...
<script type="text/javascript">
    window.addEventListener("load", () => {
        ...
        // 검색 종류 선택
        document.querySelector("#type<%=searchType%>").setAttribute("selected", "");
    }, false);
